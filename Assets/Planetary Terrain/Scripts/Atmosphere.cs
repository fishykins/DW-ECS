...
namespace AtmosphericScatteringONeils
{
    ...
    public class Atmosphere : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            atmosphereSpace = (GameObject)Instantiate(Resources.Load("Atmosphere"), planet.createScaledSpaceCopy ? position : transform.position, transform.rotation, transform);
            atmosphereSpace.transform.localScale = Vector3.one * m_outerRadius;

            atmosphereGround = (GameObject)Instantiate(Resources.Load("AtmosphereGround"), planet.createScaledSpaceCopy ? position : transform.position, transform.rotation, transform);
        ...
        void Update()
        {
            ...
            if (Vector3.Distance(transform.position, Camera.main.transform.position) < (planet.createScaledSpaceCopy ? m_outerRadius * planet.scaledSpaceFactor : m_outerRadius))
            {
        ...
        void InitMaterial(Material mat)
        {
            ...
            mat.SetVector("v3LightPos", m_sun.transform.forward * -1.0f);
            mat.SetVector("v3Translate", planet.createScaledSpaceCopy ? planet.scaledSpaceCopy.transform.position : transform.localPosition);
        }
    }
}
